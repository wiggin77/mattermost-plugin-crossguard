...
```
Outbound: OnSharedChannelsSyncMsg -> buildOutboundEnvelopes (one per post + optional metadata) -> publishToOutboundConn (Epoch, Sequence stamped per envelope) -> QueueProvider.Publish()
...
**Outbound path** (`hooks.go` -> `transport.go` -> `connections.go`): The plugin implements the shared-channels framework hooks (`OnSharedChannelsSyncMsg`, `OnSharedChannelsAttachmentSyncMsg`, `OnSharedChannelsProfileImageSyncMsg`, `OnSharedChannelsPing`). On a content sync, the upstream `*mmModel.SyncMsg` is fanned out by `buildOutboundEnvelopes` into one envelope per post plus an optional metadata envelope for non-post content (orphan reactions, memberships, statuses). Each envelope carries at most one post on the wire (`<Post>` is bare under `<SyncMsg>`, never wrapped), so compliance content inspection rejects with minimum blast radius. `publishToOutboundConn` stamps `Epoch` (one per plugin process, generated at `OnActivate`) and `Sequence` (per `(connName, channelID)` monotonic counter via `KVStore.BumpSequenceCounter`) on every `sync_msg` envelope.
