...
docker-integration-test: docker-check
	...
	echo "Getting team IDs..." && \
	LOOP_TEAM=$$(curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/loop \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	TEST_TEAM=$$(curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/test \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	echo "Creating local-loopback channels..." && \
	LOOP_CH=$$(curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"team_id":"'"$$LOOP_TEAM"'","name":"local-loopback","display_name":"Local Loopback","type":"O"}' \
		2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null || \
		curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/loop/channels/name/local-loopback \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	echo "  Server A loop/local-loopback channel ($$LOOP_CH)" && \
	LB_CH=$$(curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"team_id":"'"$$TEST_TEAM"'","name":"local-loopback","display_name":"Local Loopback","type":"O"}' \
		2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null || \
		curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/test/channels/name/local-loopback \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	...
	USERAA_ID=$$(curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/users/username/useraa \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$LOOP_CH/members \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"user_id":"'"$$USERA_ID"'"}' >/dev/null 2>&1 || true && \
	curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$LB_CH/members \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"user_id":"'"$$USERA_ID"'"}' >/dev/null 2>&1 || true && \
	...
	sleep 3 && \
	LB_FOUND=$$(curl -sf "http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$LB_CH/posts?per_page=10" \
	...
	XML_LOOP_CH=$$(curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"team_id":"'"$$LOOP_TEAM"'","name":"xml-loopback","display_name":"XML Loopback","type":"O"}' \
		2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null || \
		curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/loop/channels/name/xml-loopback \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	echo "  Server A loop/xml-loopback channel ($$XML_LOOP_CH)" && \
	XML_LB_CH=$$(curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"team_id":"'"$$TEST_TEAM"'","name":"xml-loopback","display_name":"XML Loopback","type":"O"}' \
		2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null || \
		curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/test/channels/name/xml-loopback \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	echo "  Server A test/xml-loopback channel ($$XML_LB_CH)" && \
	echo "Adding users to xml-loopback channels..." && \
	USERA_ID=$$(curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/users/username/usera \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$XML_LOOP_CH/members \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"user_id":"'"$$USERA_ID"'"}' >/dev/null 2>&1 || true && \
	curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$XML_LB_CH/members \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"user_id":"'"$$USERA_ID"'"}' >/dev/null 2>&1 || true && \
	...
	sleep 3 && \
	XML_FOUND=$$(curl -sf "http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$XML_LB_CH/posts?per_page=10" \
...
docker-azure-smoke-test: docker-check
	...
	AZ_LOOP_CH=$$(curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"team_id":"'"$$LOOP_TEAM"'","name":"azure-loopback","display_name":"Azure Queue Loopback","type":"O"}' \
		2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null || \
		curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/loop/channels/name/azure-loopback \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	echo "  Server A loop/azure-loopback channel ($$AZ_LOOP_CH)" && \
	AZ_LB_CH=$$(curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"team_id":"'"$$TEST_TEAM"'","name":"azure-loopback","display_name":"Azure Queue Loopback","type":"O"}' \
		2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null || \
		curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/test/channels/name/azure-loopback \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	echo "  Server A test/azure-loopback channel ($$AZ_LB_CH)" && \
	echo "Adding users to azure-loopback channels..." && \
	USERA_ID=$$(curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/users/username/usera \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$AZ_LOOP_CH/members \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"user_id":"'"$$USERA_ID"'"}' >/dev/null 2>&1 || true && \
	curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$AZ_LB_CH/members \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"user_id":"'"$$USERA_ID"'"}' >/dev/null 2>&1 || true && \
	echo "  usera added to both azure-loopback channels" && \
	echo "Adding Azure loopback connections to Server A config..." && \
	EXISTING=$$(curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/config \
	...
	EXISTING_IB=$$(curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/config \
	...
	sleep 3 && \
	AZ_FOUND=$$(curl -sf "http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$AZ_LB_CH/posts?per_page=10" \
	...
	TOKEN_USERA=$$(curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/users/login \
		-d '{"login_id":"usera","password":"password"}' -i 2>/dev/null \
		| grep -i '^Token:' | awk '{print $$2}' | tr -d '\r') && \
	AZ_LOOP_CH=$$(curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/loop/channels/name/azure-loopback \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	AZ_LB_CH=$$(curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/test/channels/name/azure-loopback \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	AZ_FILE_ID=$$(date +%s)-$$$$-azf && \
	echo "Uploading sample.pdf to Server A and posting via Azure connection..." && \
	FILE_UPLOAD=$$(curl -sf -X POST "http://$(MM_HOST):$(MM_PORT_A)/api/v4/files?channel_id=$$AZ_LOOP_CH" \
		-H "Authorization: Bearer $$TOKEN_USERA" \
		-F "files=@testdata/sample.pdf" | python3 -c "import sys,json; print(json.load(sys.stdin)['file_infos'][0]['id'])") && \
	curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/posts \
		-H "Authorization: Bearer $$TOKEN_USERA" \
	...
	sleep 4 && \
	AZ_FILE_FOUND=$$(curl -sf "http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$AZ_LB_CH/posts?per_page=10" \
...
docker-azure-blob-smoke-test: docker-check
	...
	AZB_LOOP_CH=$$(curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"team_id":"'"$$LOOP_TEAM"'","name":"azure-blob-loopback","display_name":"Azure Blob Loopback","type":"O"}' \
		2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null || \
		curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/loop/channels/name/azure-blob-loopback \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	echo "  Server A loop/azure-blob-loopback channel ($$AZB_LOOP_CH)" && \
	AZB_LB_CH=$$(curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"team_id":"'"$$TEST_TEAM"'","name":"azure-blob-loopback","display_name":"Azure Blob Loopback","type":"O"}' \
		2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null || \
		curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/test/channels/name/azure-blob-loopback \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	echo "  Server A test/azure-blob-loopback channel ($$AZB_LB_CH)" && \
	echo "Adding users to azure-blob-loopback channels..." && \
	USERA_ID=$$(curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/users/username/usera \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$AZB_LOOP_CH/members \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"user_id":"'"$$USERA_ID"'"}' >/dev/null 2>&1 || true && \
	curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$AZB_LB_CH/members \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"user_id":"'"$$USERA_ID"'"}' >/dev/null 2>&1 || true && \
	...
	sleep 6 && \
	AZB_FOUND=$$(curl -sf "http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$AZB_LB_CH/posts?per_page=10" \
	...
	TOKEN_USERA=$$(curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/users/login \
		-d '{"login_id":"usera","password":"password"}' -i 2>/dev/null \
		| grep -i '^Token:' | awk '{print $$2}' | tr -d '\r') && \
	AZB_LOOP_CH=$$(curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/loop/channels/name/azure-blob-loopback \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	AZB_LB_CH=$$(curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/test/channels/name/azure-blob-loopback \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	AZB_FILE_ID=$$(date +%s)-$$$$-azbf && \
	echo "Uploading sample.pdf to Server A and posting via azure-blob connection..." && \
	FILE_UPLOAD=$$(curl -sf -X POST "http://$(MM_HOST):$(MM_PORT_A)/api/v4/files?channel_id=$$AZB_LOOP_CH" \
		-H "Authorization: Bearer $$TOKEN_USERA" \
		-F "files=@testdata/sample.pdf" | python3 -c "import sys,json; print(json.load(sys.stdin)['file_infos'][0]['id'])") && \
	curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/posts \
		-H "Authorization: Bearer $$TOKEN_USERA" \
	...
	sleep 10 && \
	AZB_FILE_FOUND=$$(curl -sf "http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$AZB_LB_CH/posts?per_page=10" \
...
docker-servicebus-smoke-test: docker-check servicebus-probe-run
	...
	SB_LOOP_CH=$$(curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"team_id":"'"$$LOOP_TEAM"'","name":"servicebus-loopback","display_name":"Service Bus Loopback","type":"O"}' \
		2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null || \
		curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/loop/channels/name/servicebus-loopback \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	echo "  Server A loop/servicebus-loopback channel ($$SB_LOOP_CH)" && \
	SB_LB_CH=$$(curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"team_id":"'"$$TEST_TEAM"'","name":"servicebus-loopback","display_name":"Service Bus Loopback","type":"O"}' \
		2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null || \
		curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/teams/name/test/channels/name/servicebus-loopback \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	echo "  Server A test/servicebus-loopback channel ($$SB_LB_CH)" && \
	USERA_ID=$$(curl -sf http://$(MM_HOST):$(MM_PORT_A)/api/v4/users/username/usera \
		-H "Authorization: Bearer $$TOKEN_A" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])") && \
	curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$SB_LOOP_CH/members \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"user_id":"'"$$USERA_ID"'"}' >/dev/null 2>&1 || true && \
	curl -sf -X POST http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$SB_LB_CH/members \
		-H "Authorization: Bearer $$TOKEN_A" -H "Content-Type: application/json" \
		-d '{"user_id":"'"$$USERA_ID"'"}' >/dev/null 2>&1 || true && \
	...
	sleep 3 && \
	SB_FOUND=$$(curl -sf "http://$(MM_HOST):$(MM_PORT_A)/api/v4/channels/$$SB_LB_CH/posts?per_page=10" \
