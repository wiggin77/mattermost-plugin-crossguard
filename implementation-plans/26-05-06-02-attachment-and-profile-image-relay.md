...
- Three header constants exist in `server/connections.go:14-16`: `X-Post-Id`, `X-Conn-Name`, `X-Filename`.
...
| Out-of-order delivery | The framework's optimistic cursor retries the hook when the plugin returns error. The plugin's inbound side returns error from `WatchFiles` handlers when prerequisites are missing (e.g. profile image arrives before the synthetic user, attachment arrives before the post). | Same retry mechanism the message path uses; no new retry queue needed. |
...
A non-nil error keeps the framework cursor at the prior position. The framework will retry the call on the next sync cycle. `ReceiveSharedChannelAttachmentSyncMsg` is idempotent (framework returns the existing FileInfo if the data is already present), so retries are safe.
...
7. `provider.UploadFile(p.ctx, key, data, headers)`.
...
1. Pull `team := headers[headerTeamName]`, `chan := headers[headerChanName]`, `postID := headers[headerPostID]`, `fileInfoB64 := headers[headerFileInfo]`.
...
6. Verify the channel is linked for `connName` via `kvstore.GetChannelConnections` + `hasInboundConnection`. If not: return nil (intentional unlink, never retry).
...
- **NATS** (server/nats_provider.go): uses JetStream Object Store with a 1-hour TTL on the bucket. Profile image overwrites work natively (Object Store keys are name-based). Existing file path is correct as-is.
...
- `getFile` injection point in `newAzureBlobProvider` and the closure in `connections.go:189-195` (callers will use `p.API.GetFile` directly from hooks.go)
...
Reserved block for new file-relay codes. Each file owns a 1000-range; `hooks.go` is 10000-10999 and `inbound.go` is 15000-15999. Append at next free integer in each block:
...
- **Profile image local-user resolution**: depends on the framework exposing a way to look up a synthetic user by `(remoteID, remoteUserID)`. If the framework does not have a clean lookup, we may need to maintain our own KV mapping. Resolved during implementation — return error and retry while we figure it out is the safe fallback.
...
- **NATS Object Store TTL**: 1 hour. If a receiver is offline for longer than that, files are lost while messages (which have no TTL on transport) survive. Acceptable per "fire-and-forget" stance, but document in the operator runbook.
