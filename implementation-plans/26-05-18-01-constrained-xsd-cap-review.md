...
Caps below carry a uniform 50% headroom margin over the original
analysis (which was either exact-match or generous against upstream),
with two exceptions: `MattermostIdType` is held at exactly 26 because
the value is generated by `mmModel.NewId()` and is invariant, and
`MessageTextType` is held at 1 MiB because it already sits 16x above
upstream and there is no operational reason to widen it further.
Where a `xs:pattern` quantifier mirrored the `xs:maxLength`, the
pattern quantifier was extended in lockstep so the effective cap
actually moves (see `schema/crossguard.xsd`).

| Schema type | Cap | Used by | Upstream limit | Verdict |
|---|---:|---|---:|---|
| `MattermostIdType` | 26 | All `*Id` fields, `Epoch` | `mmModel.NewId()` = 26 chars | Exact match — invariant, no headroom |
| `SlugType` | 96 | `ConnName`, `TeamName`, `ChannelName` | `TeamNameMaxLength` = 64, `ChannelNameMaxLength` = 64 | +50% headroom over upstream — fine |
| `UsernameType` | 96 | `Username`, `RemoteUsername` (prop) | `UserNameMaxLength` = 64 | +50% headroom over upstream — fine |
| `DisplayNameType` | 192 | `OverrideUsername`, `WebhookDisplayName` | No documented upstream cap; webhook display name is free-form text | Generous (+50% headroom) — fine |
| `EmailType` | 381 | `Email`, `RemoteEmail` (prop) | `UserEmailMaxLength` = 128 | +50% over RFC 5321 (254) — fine |
| `LocaleType` | 53 | `Locale` | BCP-47 codes ≤10 chars in practice | Generous (+50% headroom; pattern quantifiers also extended) — fine |
| `RolesType` | 1536 | `Roles` | `UserRolesMaxLength` = 256 | Generous (+50% headroom; pattern quantifier extended) — fine |
| `EmojiNameType` | 96 | `Reaction.EmojiName`, `OverrideIconEmoji` | `EmojiNameMaxLength` = 64 | +50% headroom over upstream — fine |
| `UrlType` | 3072 | `OverrideIconURL` | No documented cap; browser-safe URL limit is 2048 | +50% over browser-safe limit — fine |
| `ShortTextType` | 192 | `Nickname`, `FirstName`, `LastName` | `UserNicknameMaxRunes` = 64, `UserFirstNameMaxRunes` = 64, `UserLastNameMaxRunes` = 64 | Generous (+50% headroom) — fine |
| `MediumTextType` | 1536 | `Position`, `BotDescription`, `CustomStatus` | `UserPositionMaxRunes` = 128; `BotDescription`/`CustomStatus` no documented cap | Generous (+50% headroom) — fine |
| `HashtagsType` | 1536 | `Hashtags` | `PostHashtagsMaxRunes` = 1000 | Comfortably above upstream (+50% headroom) — fine |
| `MessageTextType` | 1048576 (1 MiB) | `Message` | `PostMessageMaxBytesV2` = 65535 (64 KiB) | Schema is 16x upstream — held, no further headroom |
| `PostTypeStringType` | 96 | `Post.Type` | Column traditionally `varchar(26)`; plugin/custom types may extend | Generous (+50% headroom) — fine |
| `MapKeyType` | 192 | `StringMap.Entry/@key` (Timezone) | Timezone keys ≤30 chars (`automaticTimezone`, etc.) | Generous (+50% headroom) — fine |
| `MapValueType` | 6144 | `StringMap.Entry/@value` (Timezone) | Timezone values are timezone names (e.g., `America/Argentina/Buenos_Aires`) ≤50 chars | Generous (+50% headroom) — fine |
| `MentionKeyType` | 384 | `MentionTransforms.Transform/@key` | Mention strings are `@username[:remoteID]` ≤64+suffix | Generous (+50% headroom) — fine |
| `MentionValueType` | 384 | `MentionTransforms.Transform/@value` | Same shape as key | Generous (+50% headroom) — fine |

**No simple-type cap requires negotiation.** All are at or above
upstream limits, with 50% headroom layered on every cap except
`MattermostIdType` (invariant) and `MessageTextType` (already 16x
upstream).
