...
// Azure error sanitization. Errors returned by the Azure SDK and by azidentity
// can include credential material: SAS keys and signatures (shared-key /
// connection-string mode), OAuth2 form bodies with `client_secret=...` (SP
// mode), AAD bearer tokens, and access/refresh tokens in JSON. Every site
// that logs or returns an SDK error to a user-facing path MUST run the
// string through sanitizeAzureError first.
//
...
// the same SDK error machinery in Queue and Blob providers (via azidentity
// token-acquisition failures), so the regex set lives here in shared scope
// and is applied by all three providers and the test-connection API paths.
...
// in format strings without guarding for nil. Apply at every boundary where
// an SDK error flows into a log line or a user-facing string: provider
// constructors, Publish/Send paths, Subscribe ack sites, Close, and the
// test-connection API handlers.
